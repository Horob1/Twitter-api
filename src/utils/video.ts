...
const MAXIMUM_BITRATE_480P = 2 * 10 ** 6 // 2Mbps
...
export const checkVideoHasAudio = async (filePath: string) => {
  const [{ $ }, slash] = await Promise.all([import('zx'), import('slash').then((module) => module.default)])
...
const encodeMax720 = async ({
  ...
  // Đoạn mã hóa cho các độ phân giải khác nhau
  args.push(
    '-s:v:0',
    `${getWidth(480, resolution)}x480`,
    '-c:v:0',
...
const encodeMax1080 = async ({
  ...
  // Các tham số cho độ phân giải 480p
  args.push('-s:v:0', `${getWidth(480, resolution)}x480`, '-c:v:0', 'libx264', '-b:v:0', `${bitrate[480]}`)

  // Các tham số cho độ phân giải 720p
  args.push('-s:v:1', `${getWidth(720, resolution)}x720`, '-c:v:1', 'libx264', '-b:v:1', `${bitrate[720]}`)

  // Các tham số cho độ phân giải 1080p
  args.push('-s:v:2', `${getWidth(1080, resolution)}x1080`, '-c:v:2', 'libx264', '-b:v:2', `${bitrate[1080]}`)
...
const encodeMax1440 = async ({
  ...
  if (isHasAudio) {
    ...
    args.push('-map', '0:0', '-map', '0:0', '-map', '0:0', '-map', '0:0', '-map', '0:0')
  }
  ...
  // Các tham số cho độ phân giải 1440p
  args.push('-s:v:3', `${getWidth(1440, resolution)}x1440`, '-c:v:3', 'libx264', '-b:v:3', `${bitrate[1440]}`)
...
const encodeMaxOriginal = async ({
  ...
  // Các tham số cho độ phân giải gốc
  args.push('-s:v:3', `${resolution.width}x${resolution.height}`, '-c:v:3', 'libx264', '-b:v:3', `${bitrate.original}`)
